...
class AdapterFingerPrint : public MetaspaceObj {
 ...
 public:
  ...
  int size() const { return heap_word_size(sizeof(AdapterFingerPrint) + (_length > _compact_int_count ? _length - _compact_int_count : 0)); }
...
AdapterHandlerEntry* AdapterHandlerLibrary::create_simple_adapter(AdapterBlob*& new_adapter,
  ...
  assert(entry == nullptr || entry->is_shared(), "Non null AdapterHandlerEntry should be in the AOT cache");
  assert(entry == nullptr || !entry->is_linked(), "Simple adapter shouldn't already be linked here");
