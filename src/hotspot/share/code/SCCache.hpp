...
struct SCnmethod {
 ...
 public:
  ...
  int content_size() { return _content_size; }
  int code_offset() { return _code_offset; }
  int stub_offset() { return _stub_offset; }
  int metadata_offset() { return _metadata_offset; }
#if INCLUDE_JVMCI
  int jvmci_data_offset() { return _jvmci_data_offset; }
#endif
  int oops_count() { return _oops_count; }
  int metadata_count() { return _metadata_count; }

  int frame_complete_offset() { return _frame_complete_offset; }
  int frame_size() { return _frame_size; }

  uint compute_flags(nmethod* nm);
  bool has_unsafe_access() { return _flags & HAS_UNSAFE_ACCESS; }
  bool has_method_handle_invokes() { return _flags & HAS_MH_INVOKE; }
  bool has_wide_vectors() { return _flags & HAS_WIDE_VECTORS; }
  bool has_monitors() { return _flags & HAS_MONITORS; }
  bool has_scoped_access() { return _flags & HAS_SCOPED_ACCESS; }
  bool has_clinit_barriers() { return _flags & HAS_CLINIT_BARRIERS; }

  uint16_t entry_offset() { return _entry_offset; }
  uint16_t verified_entry_offset() { return _verified_entry_offset; }
  int skipped_instructions_size() { return _skipped_instructions_size; }
  int exception_offset() { return _exception_offset; }
  int deopt_handler_offset() { return _deopt_handler_offset; }
  int deopt_mh_handler_offset() { return _deopt_mh_handler_offset; }
  int16_t unwind_handler_offset() { return _unwind_handler_offset; }

  int immutable_data_size() { return _immutable_data_size; }
  int dependencies_size() { return _dependencies_size; }
  int nul_chk_table_offset() { return _nul_chk_table_offset; }
  int handler_table_offset() { return _handler_table_offset; }
  int scopes_pcs_offset() { return _scopes_pcs_offset; }
  int scopes_data_offset() { return _scopes_data_offset; }
#if INCLUDE_JVMCI
  int speculation_offset() { return _speculation_offset; }
  int speculations_len() { return _speculations_len; }
#endif
  ...
  uint relocation_data_offset() { return _relocation_data_offset; }
  uint content_offset() { return _content_offset; }
  uint oop_metadata_offset() { return _oop_metadata_offset; }
  uint oop_map_offset() { return _oop_map_offset; }
  uint immutable_data_offset() { return _immutable_data_offset; }
  uint reloc_immediates_offset() { return _reloc_immediates_offset; }
  uint extra_reloc_offset() { return _extra_reloc_offset; }
  void print_on(outputStream* st);
};
