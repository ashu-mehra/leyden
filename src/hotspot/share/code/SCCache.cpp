...
bool SCCReader::read_code_v1(CodeBlob* codeBlob, uint code_offset) {
  ...
  SCCache::copy_bytes((const char*)relocations, (address)codeBlob->relocation_begin(), sc_blob->relocation_size());
  ...
  SCCache::copy_bytes((const char*)contents, codeBlob->content_begin(), sc_blob->content_size());
...
oop SCCReader::read_oop(JavaThread* thread, const methodHandle& comp_method) {
  ...
  if (kind == DataKind::Null) {
    ...
    return (oop)Universe::non_oop_word();
...
bool SCCache::write_oop(oop obj) {
  ...
  } else if (obj == (oop)Universe::non_oop_word()) {
  ...
  while (iter.next()) {
	...
	break;
      case relocInfo::oop_type: {
        oop_Relocation* r = (oop_Relocation*)iter.reloc();
	if (r->oop_is_immediate()) {
	  r->set_value((address)oop_list.at(reloc_data[j]));
	} else {
