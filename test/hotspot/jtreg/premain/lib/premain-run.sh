...
do_test "(STEP 4 of 5) Run with $APP-dynamic.jsa and generate AOT code" \
    $JAVA -XX:SharedArchiveFile=$APP-dynamic.jsa -XX:+ReplayTraining -XX:+StoreCachedCode -XX:+StoreStubs \
        -Xlog:scc*=info,stubs=debug:file=$APP-store-sc.log::filesize=0 \
...
do_test "(STEP 5 of 5) Final production run: with $APP-dynamic.jsa and load AOT code" \
    $JAVA -XX:SharedArchiveFile=$APP-dynamic.jsa -XX:+ReplayTraining -XX:+LoadCachedCode -XX:+LoadStubs \
        -Xlog:scc=info,scc*=info,stubs=debug:file=$APP-load-sc.log::filesize=0 \
